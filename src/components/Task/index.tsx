...
function Task({ description, forTime, id, isCompleted, tillTime, title }: TaskProps) {
    ...
    return (
        <View style={styles.container}>
                        ...
                        <View style={styles.leftContainer}>
                            ...
                            <View style={styles.taskContainer}>
                                <TouchableOpacity onPress={handleCheck} activeOpacity={0.5} style={styles.checkBox}>
                                    <Image style={[styles.checkBoxImage, { display: isCompleted ? "flex" : "none" }]} source={checked} />
                                </TouchableOpacity>

                                <View style={styles.taskTextContainer}>
                                    <Text style={styles.titleText}>{title}</Text>
                                    {description && <Text style={[dynamicTextStyles(isDisplaySubTasks).descriptionText]}>
                                        {description}
                                    </Text>}
                                </View>
                            </View>
            ...
            { display: isDisplaySetting ? "flex" : "none" }]}>
                <SettingButton onClick={() => { dispatch(openModalWithParams(id)) }} title="edit task" />
                <SettingButton onClick={() => { dispatch(deleteTask(id)) }} title="delete task" />
            </View>
        </View>
